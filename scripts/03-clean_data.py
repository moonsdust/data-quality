...
selected_columns = ["package", "completeness", "freshness", "metadata", "usability", "grade"]

selected_df = the_raw_data.select(selected_columns)

# Filter to only rows that have no NONE data
filtered_df = selected_df.filter(the_raw_data["package"].is_not_null() & the_raw_data["completeness"].is_not_null() & the_raw_data["freshness"].is_not_null() & the_raw_data["metadata"].is_not_null() & the_raw_data["usability"].is_not_null() & the_raw_data["grade"].is_not_null())
